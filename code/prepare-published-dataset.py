...
def nibabel_orient_as_RAS(
    ...
    return img.as_reoriented(nibabel.orientations.ornt_transform(img_ornt, [[0, 1], [1, 1], [2, 1]]))
...
def convert_to_scaled_encoding(img, dtype, slope, inter, reset_scaling=False):
    ...
    new_arr[mask] = 0
    print(f"{numpy.count_nonzero(mask)} NaN values encoded by zero (scaled value {inter})")
    ...
    if not dry_run:
        ...
        if 'affine_precision_override' in config:
            ...
            if (numpy.allclose(affine[:3, :3], img.affine[:3, :3], rtol=1e-4)
                ...
                logger.error("Refusing to override the affine matrix, "
            ...
            if not numpy.allclose(voxel_size, img.header.get_zooms(), rtol=1e-4):
                logger.error("Refusing to override the voxel size, "
...
def prepare_published_dataset(input_dir: os.PathLike | str,
    ...
    for image_dict in config['images']:
        encode_image(image_dict, config, input_dir, output_dir,
                     dry_run=dry_run)
...
def main(argv=sys.argv):
    ...
    return prepare_published_dataset(args.input_dir, args.output_dir, args.config_yaml,
