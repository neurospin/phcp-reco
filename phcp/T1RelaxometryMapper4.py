...
def VFAregister(img_filename, output_filename, verbose):
    ...
    for i in range(1,volume_number):
        ...
        registration = ants.registration(ANTS_fixed_image,
                                         ANTS_moving_image,
...
def createCommand_T1SingleCompartmentRelaxometryMapper(fileNameVFACat,
                                           fileNameMask,
                                           fileNameTRValues,
                                           fileNameFAValues,
                                           fileNameB1,
    ...
    command = 'GkgExecuteCommand SingleCompartmentRelaxometryMapper'+\
                ' -i %s' % fileNameVFACat +\
                ' -m %s' % fileNameMask +\
                ...
                ' -stringParameters ' + str( fileNameTRValues ) +\
                ' -stringParameters ' + str( fileNameFAValues ) +\
                ' -stringParameters ' + str( fileNameB1 ) +\
        ...
        with open( fileNameTRValues, 'w') as file2:
            for metadata_filename in sorted(glob.iglob(FAFilenames)): 
                ...
                if os.path.exists(fileNameB1):
                    ...
                    file.write(str(math.radians(metadata['FlipAngle']) )+'\n') #Si pas de carte  de B1 --> convertir les angles en radians
                file2.write(str(metadata['RepetitionTime']*1000) + '\n')
    
    return None 

def create_ConfidenceMap(fileNameVFACat, fileNameFittedVFA, fileNameMask, fileNameT1nifti):
    ...
    ni_im = ni.Nifti1Image(dearrange_ArrayToFlipHeaderFormat(std_diff), metaT1nifti.affine, metaT1nifti.header)
    return ni_im


""" Correction Part """
def extract_BiasField(ants_T1img, ants_T1img_mask, verbose):   
    return ants.n4_bias_field_correction(ants_T1img, ants_T1img_mask, spline_param=20, rescale_intensities=True, return_bias_field=True, verbose = verbose)       
...
def create_NewBiasField(biasField):
    ...
    return np.where(biasField_numpy < (moy_inconsistency +2* std_inconsistency), moy_normal/biasField_numpy, 1 )
...
def runT1RelaxometryMapper( subjectDirectoryGisConversion,
                            FAFilenames,
                            outputDirectory,
                            verbose ):
    ...
    if not(os.path.exists(fileNameT1nifti)):
        ...
        command = createCommand_T1SingleCompartmentRelaxometryMapper(fileNameVFACat,
                                               fileNameMask,
                                               fileNameTRValues,
                                               fileNameFAValues,
                                               fileNameB1,
    ...
    if not(os.path.exists(fileNameConfidenceMap)):
        ni_im = create_ConfidenceMap(fileNameVFACat, fileNameFittedVFA, fileNameMask, fileNameT1nifti)
...
runT1RelaxometryMapper( options.vfadirectory,
                       options.VFAFilenames,
