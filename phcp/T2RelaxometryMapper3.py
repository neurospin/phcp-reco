...
@author: la272118
"""
import os, json, optparse, subprocess
import numpy as np
...
def createCommand_SingleCompartmentRelaxometryMapper(fileNameMSME,
                                           fileNameMask,
                                           fileNameTEValues,
...
def create_ConfidenceMap(fileNameMSME, fileNameFittedMSME, fileNameMask, fileNameT2nifti):
    ...
    ni_im = ni.Nifti1Image(dearrange_ArrayToFlipHeaderFormat(std_diff), meta_t2nifti.affine, meta_t2nifti.header)
    return ni_im

def runT2RelaxometryMapper( subjectDirectoryGisConversion,
                            MSMEFilenames,
                            outputDirectory,
                            verbose ):
    ...
    command = createCommand_SingleCompartmentRelaxometryMapper(fileNameMSME,
                                           fileNameMask,
                                           fileNameTEValues,
                                           fileNameProtonDensity,
                                           fileNameT2,
    ...
    ni_im = create_ConfidenceMap(fileNameMSME, fileNameFittedMSME, fileNameMask, fileNameT2nifti)
...
parser.add_option( '-i', '--input',
                   dest = 't2msmedirectory',
                   help = 'T2-MSME Directory' )
parser.add_option( '-m', '--msme',
                   dest = 'MSMEFilenames',
...
runT2RelaxometryMapper( options.t2msmedirectory,
