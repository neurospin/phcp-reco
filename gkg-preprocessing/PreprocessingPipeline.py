...
import os, json, sys, glob, nibabel, numpy, optparse, re
...
def runBvecAndBvalWritter(SourceDataDirectory,
                          RawDataDirecctory,
                          Subject,
                          Session,
                          DescriptionFilename):
    ...
    for bvalue in DescriptionFile.keys():
        for compteur in range(len(DescriptionFile[ bvalue ])):
            ...
            DiffDirectory = os.path.join(SourceDataDirectory, 
                                         Subject, 
                                         Session,
            ...
            Parser = BrukerInformationParser.BrukerInformationParser(DiffMethod,
                                                                     DiffAcqp,
                                                                     DiffVisu,
                                                                     False)
            
            RawdataDirectorySaver = os.path.join(RawDataDirecctory,
                                                 Subject,
                                                 Session,
                                                 "dwi")
            FileName = Subject + "_" + Session \
                        + "_acq-" + bvalue + "_run-" + str(compteur+1) \
                        +  "_dwi"
            
            Parser.saveBvecAndBval(os.path.join(RawdataDirectorySaver, FileName + ".bvec"),
                                   os.path.join(RawdataDirectorySaver, FileName + ".bval"))  
    return None

def runBvalMerger(DwiDirectory,
                  Subject,
                  Session,
                  bvalue,
                  OutputDirectory):
    ...
    BvalFiles = sorted(glob.glob(os.path.join(DwiDirectory, prefix_filename +'*.bval')))
    values = ""
...
def runBvecMerger(DwiDirectory,
                  Subject,
                  Session,
                  bvalue,
                  OutputDirectory):
    ...
    BvecFiles = sorted(glob.glob(os.path.join(DwiDirectory, prefix_filename +'*.bvec')))
    valuex = ""
    valuey = ""
    valuez = ""
...
def runRGWritter(RawDataDirectory,
                 SourceDataDirectory,
                 SaveDirectory,
                 Subject,
                 Session,
    ...
    for modality in SubSesKeyModalityValue_dict[ key ]:      
        if modality == "VFA":
            RawDataSubjectDirectory = os.path.join(RawDataDirectory, 
                                         Subject, 
                                         Session, 'anat')
        
            FileName = Subject + "_" + Session \
                        + "_flip-*" + '_'+ modality  + '.json'
        ...
        for File in glob.iglob(os.path.join(RawDataSubjectDirectory, FileName)):
            ...
            DirectoryFile = os.path.join(SourceDataDirectory, \
                                         Subject, \
                                         Session, \
                                         number, \
                                         'acqp')
...
def correct_RG(RGCorrectionDirectory,
               RGDictionary,
               RawdataDirectory,
               Subject,
               Session,
               FolderName,
               GlobSearch):
    Files_iterator = sorted(glob.iglob(os.path.join(RawdataDirectory, 
                                    Subject,
                                    Session, 
                                    FolderName, 
                                    GlobSearch)) )
    ...
    nibabel.save(ni_volume_ref, os.path.join(RGCorrectionDirectory,
                                                   Subject, 
                                                   Session, 
                                                   FolderName, 
    ...
    for filename in Files_iterator:
        ...
        volume = nibabel.load(os.path.join(RawdataDirectory,
                                                    Subject,
                                                    Session, FolderName, filename.split('/')[-1]))
        volume_arr = volume.get_fdata()
        volume_arr_corrected = volume_arr*ref/RGValue
        volume_nii = nibabel.Nifti1Image(volume_arr_corrected, volume.affine, volume.header)
        
        nibabel.save(volume_nii, os.path.join(RGCorrectionDirectory,
                                                       Subject, 
                                                       Session, 
                                                       FolderName, 
                                                       filename.split('/')[-1]))
...
def merge_volume(prefix, inputFilenames, outputFilename, isVFA):
    ...
    for volume in Iterator:
        ...
        arr = meta.get_fdata()
        if isVFA:
            res = numpy.concatenate((res, numpy.reshape([arr], meta.shape+(1,))), 
                                     axis=-1)
        else:
...
def NiiMerger(FilesDirectory, DerivativesDirectory, modality, Subject, Session):
    ...
    SaveDwiDirectory = os.path.join(DerivativesDirectory, 'gkg-Pipeline', Subject, Session)
    
    isVFA = False
    if 'VFA' == modality:
        print_message("VFA Merger")
        SaveVFADirectory = os.path.join(DerivativesDirectory, 'RGCorrection', Subject, Session)

        prefix = Subject + '_' + Session + '_' + 'flip-*' + '_VFA.nii.gz'
        inputFilenames = os.path.join(SessionDirectory,
                                      'anat',
                                      prefix)
        outputFilename = os.path.join(SaveVFADirectory,
                                      Subject + '_' + Session + '_VFA.nii.gz')
        isVFA = True
        ...
        outputFilename = os.path.join(SaveDwiDirectory,
                                      "b8000",
                                      "01-Materials",
...
def runVolumesRGCorrection(RGValuesJsonFilename,
                                 SubSesKeyModalityValue_json,
                                 Subject,
                                 Session,
                                 RawdataDirectory,
    ...
    for modality in SubSesKeyModalityValue_dict[ Id ]:
        if modality[0]=='b': #if key is a bvalue
            ...
            filtered_RGDictionary = {k: v for k, v in RGDictionary.items() if k.startswith(prefix)}
            if len(set(filtered_RGDictionary.values()))>1:
                FolderName = 'dwi'
                GlobSearch = Subject + "_" + Session \
                                + "_acq-" + modality + "_run-*"  \
                                +  "_dwi.nii.gz"
                FilesDirectory = RGCorrectionDirectory
                correct_RG(RGCorrectionDirectory,
                           RGDictionary,
                           RawdataDirectory,
                           Subject,
                           Session,
...
def main(SourcedataDirectory,
                             RawdataDirectory,
                             DerivativesDirectory,
                             SubjectKeySessionValue_json,
                             DescriptionsDirectory):
    
    SubSesKeyModalityValue_json = os.path.join(DescriptionsDirectory, 'RG_description.json')
    ...
    for subject in SubjectKeySessionValue_dict.keys():
        for session in SubjectKeySessionValue_dict[ subject ]:
            ...
            if is_bvalue(SubSesKeyModalityValue_dict[Id]):
                BvalueKeyNumberFileValue_json = os.path.join(DescriptionsDirectory, subject + '_' + session + '_description.json')
                ...
                runBvecAndBvalWritter(SourcedataDirectory,
                                          RawdataDirectory,
                                          subject,
                                          session,
                ...
                for bvalue in BvalueKeyNumberFileValue_dict.keys():
                    SessionDirectory = os.path.join(RawdataDirectory, 
                                                    subject, 
                                                    session)
                    DwiDirectory = os.path.join(SessionDirectory, 'dwi' )
                    
                    if not os.path.exists(os.path.join(DerivativesDirectory, 'gkg-Pipeline', subject, session, bvalue, "01-Materials")):
                        os.makedirs(os.path.join(DerivativesDirectory, 'gkg-Pipeline', subject, session, bvalue, "01-Materials"))

                    MaterialsDirectory = os.path.join("/neurospin/phcp", DerivativesDirectory, 'gkg-Pipeline', subject, session, bvalue, "01-Materials")
          
                    print_message("Run BvecAndBvalMerger for " + bvalue ) 
                
                    runBvalMerger(DwiDirectory, 
                                  subject,
                                  session,
                                  bvalue,
                                  MaterialsDirectory)
                    
                    runBvecMerger(DwiDirectory, 
                                  subject,
                                  session,
                                  bvalue,
                                  MaterialsDirectory)
            ...
            runVolumesRGCorrection(RGValuesJsonFilename,
                                   SubSesKeyModalityValue_json,
                                   subject,
                                   session,
                                   RawdataDirectory,
...
parser.add_option( '-s', '--sourcedata',
                   dest = 'sourcedata',
                   help = 'Sourcedata directory. ' )
parser.add_option( '-r', '--rawdata',
                   dest = 'rawdata',
                   help = 'Rawdata directory.' )
parser.add_option( '-d', '--derivatices',
                   dest = 'derivatices',
                   help = 'Dérivatives directory. ' )
parser.add_option( '-q', '--subject',
                   dest = 'subject',
                   help = 'Subject JSON filename. ' )
parser.add_option( '-p', '--descriptions',
                   dest = 'descriptions',
                   help = 'Descriptions directory. ' )


( options, args ) = parser.parse_args()

main( options.sourcedata, 
        options.rawdata,
        options.derivatices,
        options.subject,
